            ...
            FROM (
                SELECT     -- в этой выборке объединяем три внутренние выборки по расчёту столбцов витрины и применяем оконные функции для определения самой популярной категории товаров и самого популярного мастера
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, кроме столбцов с самой популярной категорией товаров у заказчика и с самым популярным мастером
                                ...
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить категорию самого популярного товара у заказчика
